...
# n = 3  # Quantum number for second excited state
# k = 1/9  # Constant relating E_1 = k * E_3 (for infinite square well, E_1 = E_3 / 9)
# E_n3 = (n * np.pi) ** 2 / (2 * m * L ** 2)  # Energy eigenvalue for n=3
# E_n1 = (1 * np.pi) ** 2 / (2 * m * L ** 2)  # Energy eigenvalue for n=1 (for reference)

# # ===================================================================================================== #
# # Analytical solution for the time-independent wave function
# def analytical_wavefunction(x, n=3, L=1.0):
...
# L_initial = L / 3  # Restrict to x in [0, 0.25]
...
# plt.plot(x, analytical_wavefunction(x.numpy(), n, L), label='Analytical wave function (n=3)', color='red')
...
# plt.title('Second Excited State (n=3) Wave Function', fontsize=20)
# plt.tight_layout()
# plt.savefig('analytical_wavefunction_n3.svg', bbox_inches='tight', pad_inches=0.1)
# plt.show()
...
#     plt.plot(x, psi_analytical, color="tab:green", linewidth=2, alpha=0.8, label="Analytical (n=3)")
...
#     plt.title('Time-Independent Wave Function (n=3)', fontsize=16)
#     plt.tight_layout()
...
#     # Compute the physics loss (Schrödinger equation: -ħ²/(2m) d²ψ/dx² = E_n3 ψ)
...
#     physics = -(hbar ** 2 / (2 * m)) * d2psi_dx2 - E_n3 * psi_hp  # Schrödinger equation with E_n3
...
#     # Energy constraint: E_1 = k * E_3
#     dx = x_physics[1] - x_physics[0]
#     kinetic_energy = -(hbar ** 2 / (2 * m)) * torch.sum(psi_hp * d2psi_dx2) * dx  # ∫ ψ (-ħ²/2m d²ψ/dx²) dx
#     normalization = torch.sum(psi_hp ** 2) * dx  # ∫ |ψ|² dx
#     E_3_pred = kinetic_energy / normalization  # Estimated E_3
#     E_1_pred = k * E_3_pred  # Predicted E_1 based on E_1 = k * E_3
#     loss4 = (1e-4) * (E_1_pred - E_n1) ** 2  # Penalize deviation from true E_1
...
#         file = f"plots/pinn_wavefunction_n3_%.8i.png" % (i + 1)
...
# save_gif_PIL("pinn_wavefunction_n3.gif", files, fps=20, loop=0)
...
for i in range(42000):
    ...
    # Orthogonality conditions: ∫ ψ_3 * ψ_1 dx = 0, ∫ ψ_3 * ψ_2 dx = 0
    psi_1_analytical = torch.tensor(analytical_wavefunction(x_physics.detach().numpy(), n=1, L=L), dtype=torch.float32)
    ortho_integral_1 = torch.sum(psi_hp * psi_1_analytical) * dx  # ∫ ψ_3 * ψ_1 dx
    loss5 = (1e-4) * ortho_integral_1 ** 2  # Penalize non-zero integral with ψ_1
    
    psi_2_analytical = torch.tensor(analytical_wavefunction(x_physics.detach().numpy(), n=2, L=L), dtype=torch.float32)
    ortho_integral_2 = torch.sum(psi_hp * psi_2_analytical) * dx  # ∫ ψ_3 * ψ_2 dx
    loss6 = (1e-4) * ortho_integral_2 ** 2  # Penalize non-zero integral with ψ_2
    
    # Total loss with weighted data loss and orthogonality
    loss = data_loss_weight * loss1 + loss2 + loss3 + loss4 + loss5 + loss6
    loss.backward()
    optimizer.step()
    ...
    if (i + 1) % 150 == 0:
        print(f"Step {i+1}: loss1 (data) = {loss1.item():.6f}, loss2 (physics) = {loss2.item():.6f}, "
              f"loss3 (boundary) = {loss3.item():.6f}, loss4 (energy) = {loss4.item():.6f}, "
              f"loss5 (ortho psi_1) = {loss5.item():.6f}, loss6 (ortho psi_2) = {loss6.item():.6f}, "
              f"total loss = {loss.item():.6f}")
